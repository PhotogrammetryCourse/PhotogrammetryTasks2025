...
// TODO включите Bundle Adjustment (но из любопытства посмотрите как ведет себя
// реконструкция без BA например для saharov32 без BA)
#define ENABLE_BA 0

// TODO когда заработает при малом количестве фотографий - увеличьте это
// ограничение до 100 чтобы попробовать обработать все фотографии (если же
// успешно будут отрабаывать только N фотографий - отправьте PR выставив здесь
// это N)
#define NIMGS_LIMIT                                                            \
  10 // сколько фотографий обрабатывать (можно выставить меньше чтобы ускорить
...
#define ENABLE_INSTRINSICS_K1_K2                                               \
  1 // TODO учитывать ли радиальную дисторсию - коэффициенты k1, k2 попробуйте с
    // ним и и без saharov32, заметна ли разница?
#define INTRINSIC_K1_K2_MIN_IMGS                                               \
  7 // начиная со скольки камер начинать оптимизировать k1, k2

// TODO попробуйте повыключать эти фильтрации выбросов, насколько изменился
// результат?
...
// достаточно чтобы у вас работало на этом датасете, тестирование на Travis CI
// тоже ведется на нем
#define DATASET_DIR "saharov32"
#define DATASET_DOWNSCALE                                                      \
  1 // картинки уже уменьшены в 4 раза (оригинальные вы можете скачать по ссылке
    // из saharov32/LINK.txt)
#define DATASET_F (1585.5 / DATASET_DOWNSCALE)

// но если любопытно - для экспериментов предлагаются еще дополнительные
// датасеты скачайте их фотографии в папку data/src/datasets/DATASETNAME/ по
// ссылке из файла LINK.txt в папке датасета:

// saharov32 и herzjesu25 - приятные датасеты, вероятно их оба получится
// выравнять целиком
// #define DATASET_DIR                  "herzjesu25"
// #define DATASET_DOWNSCALE            2 // для ускорения SIFT
// #define DATASET_F                    (2761.5 / DATASET_DOWNSCALE) // see
// herzjesu25/K.txt
// TODO почему фокальная длина меняется от того что мы уменьшаем картинку?
// почему именно в такой пропорции? может надо домножать? или делить на
// downscale^2 ?

// но temple47 - не вышло, я не разобрался в чем с ним проблема, может быть
// слишком мало точек, может критерии фильтрации выкидышей для него слишком
// строги
// #define DATASET_DIR                  "temple47"
// #define DATASET_DOWNSCALE            1
// #define DATASET_F                    (1520.4 / DATASET_DOWNSCALE) // see
// temple47/README.txt about K-matrix (i.e. focal length = K11 from
// templeR_par.txt)
...
ENABLE_OUTLIERS_FILTRATION_COLINEAR 0
 */
// и в целом все плохо... у меня не получилось выравнять этот датасет нашим
// простым прототипом
//________________________________________________________________________________

namespace {

vector3d relativeOrientationAngles(const matrix3d &R0, const vector3d &O0,
...
TEST(SFM, ReconstructNViews) {
  ...
  // Чтобы было проще - картинки упорядочены заранее в файле
  // data/src/datasets/DATASETNAME/ordered_filenames.txt
  std::vector<cv::Mat> imgs;
  std::vector<std::string> imgs_labels;
  {
    std::ifstream in(std::string("data/src/datasets/") + DATASET_DIR +
    ...
    for (int i = 0; i < nimages; ++i) {
      ...
      in >> img_name;
      std::string img_path =
          std::string("data/src/datasets/") + DATASET_DIR + "/" + img_name;
  ...
  // сверяем что все картинки одинакового размера (мы ведь предполагаем что их
  // снимала одна и та же камера с одними и те же интринсиками)
  for (const auto &img : imgs) {
    rassert(img.cols == imgs[0].cols && img.rows == imgs[0].rows,
            34125412512512);
  }
...
#pragma omp critical
      {
        ++ndone;
        if (inliers > 0) {
          std::cout << to_percent(ndone, n_imgs * (n_imgs - 1)) + "% - Cameras "
                    << i << "-" << j << " (" << imgs_labels[i] << "-"
                    << imgs_labels[j] << "): " << inliers << " matches"
                    << std::endl;
        }
      }
  ...
  {
    std::cout << "Initial alignment from cameras #0 and #1 (" << imgs_labels[0]
              << ", " << imgs_labels[1] << ")" << std::endl;
    ...
    generateTiePointsCloud(tie_points, tracks, keypoints, imgs, aligned,
                           ...
                           tie_points_colors);
    phg::exportPointCloud(tie_points_and_cameras,
                          std::string("data/debug/test_sfm_ba/") + DATASET_DIR +
...
public:
  ...
  template <typename T>
  bool operator()(
      const T *camera_extrinsics, // положение камеры:   [6] = {translation[3],
                                  // rotation[3]} (разное для всех кадров, т.к.
                                  // каждая фотография со своего ракурса)
      const T
          *camera_intrinsics, // внутренние калибровочные параметры камеры: [5]
                              // = {k1, k2, f, cx, cy} (одни и те же для всех
                              // кадров, т.к. снято на одну и ту же камеру)
      const T *point_global, // 3D точка: [3]  = {x, y, z}
      T *residuals) const {  // невязка:  [2]  = {dx, dy}
    // TODO реализуйте функцию проекции, все нужно делать в типе T чтобы
    // ceres-solver мог под него подставить как Jet (очень рекомендую посмотреть
    // Jet.h - как класная статья из википедии!), так и double
    ...
    return true;
    // TODO сверьте эту функцию с вашей реализацией проекции в
    // src/phg/core/calibration.cpp (они должны совпадать)
  }
...
void printCamera(double *camera_intrinsics) {
  std::cout << "camera: k1=" << camera_intrinsics[0]
            << ", k2=" << camera_intrinsics[1] << ", "
            << "f=" << camera_intrinsics[2] << ", "
            << "cx=" << camera_intrinsics[3] << ", cy=" << camera_intrinsics[4]
            << std::endl;
}
...
void runBA(std::vector<vector3d> &tie_points, std::vector<Track> &tracks,
  ...
  // TODO: преобразуйте calib в блок параметров камеры (ее внутренних
  // характеристик) для оптимизации в BA
  ...
  for (size_t camera_id = 0; camera_id < ncameras; ++camera_id) {
    matrix3d R;
    vector3d O;
    // Декомпозируем на матрицу поворота (локальная система камеры -> глобальная
    ...
    // AngleAxisToRotationMatrix оперирует R матрицей у которой в памяти подряд
    // идут колонки а не строчки: > Conversions between 3x3 rotation matrix (in
    // >>>column major order<<<) and > axis-angle rotation representations.
    // Templated for use with autodifferentiation. поэтому нужно
    // транспонировать:
    matrix3d Rt = R.t();
  ...
  // Создаем невязки для всех проекций 3D точек в камеры (т.е. для всех
  // наблюдений этих ключевых точек)
    ...
    for (size_t ci = 0; ci < track.img_kpt_pairs.size(); ++ci) {
      ...
      ceres::CostFunction *keypoint_reprojection_residual =
          new ceres::AutoDiffCostFunction<
              ReprojectionError,
              2, // количество невязок (размер искомого residual массива
                 // переданного в функтор, т.е. размерность искомой невязки, у
                 // нас это dx, dy (ошибка проекции по обеим осям)
              6, 5, 3> // число параметров в каждом блоке параметров, у нас три
                       // блок параметров (внешние параметры камеры[6],
                       // внутренние параметры камеры[5] и 3D точка)
      ...
      if (!track.disabled) {
        problem.AddResidualBlock(
            keypoint_reprojection_residual, new ceres::HuberLoss(3.0 * sigma),
            camera_extrinsics, camera_intrinsics, point3d_params);
      } else {
        reprojection_residuals_for_deletion.push_back(
            keypoint_reprojection_residual); // если мы не передали невязку в
                                             // ceres-solver, то за его lifetime
                                             // ответственны все еще мы
      }
    }
  }
  ...
  for (size_t camera_id = 0; camera_id < ncameras; ++camera_id) {
    ...
    std::cout << "    Camera #" << camera_id
              ...
              << "(" << ninls << "/" << nproj
              << ") with MSE=" << (cameras_inliers_mse[camera_id] / ninls)
              << std::endl;
  }
  ...
  if (ncameras < INTRINSICS_CALIBRATION_MIN_IMGS) {
    ...
    if (ncameras < INTRINSIC_K1_K2_MIN_IMGS) {
      problem.SetParameterization(camera_intrinsics,
                                  new ceres::SubsetParameterization(5, {0, 1}));
    }
  }
  ...
  std::cout << "After BA ";
  printCamera(camera_intrinsics);
  // TODO преобразуйте параметры камеры в обратную сторону, чтобы последующая
  // резекция учла актуальное представление о пространстве: calib.* =
  // camera_intrinsics[*];
  ...
  for (size_t i = 0; i < tie_points.size(); ++i) {
    ...
    for (size_t ci = 0; ci < track.img_kpt_pairs.size(); ++ci) {
      ...
      if (ENABLE_OUTLIERS_FILTRATION_COLINEAR && ENABLE_BA) {
        ...
        // чтобы не было странных точек улетающих на бесконечность (например
        // чтобы угол был хотя бы 2.5 градуса) should_be_disabled = true;
      }
  ...
  std::cout << "After BA tie poits: "
            ...
            << to_percent(n_old_outliers + n_new_outliers, tie_points.size())
            << "% total outliers" << std::endl;
  std::cout << "After BA projections: " << to_percent(inliers, nprojections)
            << "% inliers with MSE=" << (inliers_mse / inliers) << std::endl;
  for (size_t camera_id = 0; camera_id < ncameras; ++camera_id) {
    size_t ninls = cameras_inliers[camera_id];
    size_t nproj = cameras_nprojections[camera_id];
  ...
  for (auto ptr : reprojection_residuals_for_deletion) {
    delete ptr; // т.к. мы не отдали указатель в ceres-solver - мы ответственны
                // за его lifetime - надо самим деаллоцировать
  }
}
