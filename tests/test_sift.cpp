...

    {
        for (int method = 0; method < 3; ++method) { // тестируем три метода: OpenCV ORB, OpenCV SIFT, ваш SIFT
            ...
            if (angle_diff_sum != 0.0) {
                ...
                // Q: почему SIFT менее точно угадывает средний угол отклонения? изменяется ли ситуация если выкрутить параметр ORIENTATION_VOTES_PEAK_RATIO=0.999? почему?
                // A: Потому что угол получается из гистограмм, в которых у нас ограниченное количество корзин.
                //    Из-за этого просто точность ограничена
                //    Выкручивание порога должно только ухудшить ситуацию, так как если раньше для дублей точек с разными ориентациями хотя бы одной могли найти матч,
                //    То теперь - нет.
                //    Должно улучшить ситуацию увеличение числа корзин, но проверять не буду.
            }
