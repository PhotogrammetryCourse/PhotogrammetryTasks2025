...
cv::Mat getHomography(const cv::Mat &img1, const cv::Mat &img2) {
  ...
  phg::DescriptorMatcher::filterMatchesRatioTest(knn_matches, good_matches);
  {
    ...
    phg::DescriptorMatcher::filterMatchesClusters(good_matches, keypoints1,
                                                  keypoints2, tmp);
  ...
  {
    std::vector<DMatch> tmp;
    phg::filterMatchesGMS(good_matches, keypoints1, keypoints2, img1.size(),
                          img2.size(), tmp);
    std::swap(tmp, good_matches);
  }
#endif
...
void evaluateStitching(const cv::Mat &img1, const cv::Mat &img2,
                       double &keypoints_rmse, double &color_rmse,
...
namespace {
void testStitching(const cv::Mat &img1, const cv::Mat &img2,
                   const std::vector<cv::KeyPoint> &keypoints1,
                   const std::vector<cv::KeyPoint> &keypoints2,
                   const cv::Mat &descriptors1, const cv::Mat &descriptors2) {
  double rmse_kpts, rmse_color;
  evaluateStitching(img1, img2, rmse_kpts, rmse_color, keypoints1, keypoints2,
                    descriptors1, descriptors2);

  std::cout << "keypoints RMSE: " << rmse_kpts << ", color RMSE: " << rmse_color
            << std::endl;
...
void testStitchingMultipleDetectors(const cv::Mat &img1, const cv::Mat &img2) {
  {
    ...
    testStitching(img1, img2, keypoints1, keypoints2, descriptors1,
                  descriptors2);
  }
...
bool matcheq(const cv::DMatch &lhs, const cv::DMatch &rhs) {
  return std::tie(lhs.trainIdx, lhs.queryIdx, lhs.imgIdx) ==
         std::tie(rhs.trainIdx, rhs.queryIdx, rhs.imgIdx);
}

void evaluateMatching(const cv::Mat &img1, const cv::Mat &img2,
                      const std::vector<cv::KeyPoint> &keypoints1,
                      const std::vector<cv::KeyPoint> &keypoints2,
                      const cv::Mat &descriptors1, const cv::Mat &descriptors2,
                      double &nn_score, double &nn2_score, double &nn_score_cv,
                      double &nn2_score_cv, double &time_my, double &time_cv,
                      double &time_bruteforce, double &time_bruteforce_gpu,
                      double &good_nn, double &good_ratio,
                      double &good_clusters, double &good_ratio_and_clusters,
                      bool do_bruteforce) {
  using namespace cv;
  ...
  std::vector<std::vector<DMatch>> knn_matches_flann, knn_matches_flann_cv,
  ...
  for (int i = 0; i < (int)knn_matches_bruteforce_gpu.size(); ++i) {
    ASSERT_EQ(knn_matches_bruteforce_gpu[i].size(),
              knn_matches_bruteforce[i].size());
    for (int j = 0; j < (int)knn_matches_bruteforce_gpu[i].size(); ++j) {
      ASSERT_TRUE(matcheq(knn_matches_bruteforce_gpu[i][j],
                          knn_matches_bruteforce[i][j]));
    }
  }
#endif
    ...
    for (int i = 0; i < descriptors1.rows; ++i) {
      if (knn_matches_bruteforce[i][0].queryIdx != i) {
        throw std::runtime_error(
            "invalid DMatch queryIdx for knn_matches_bruteforce");
      }
      if (knn_matches_flann[i][0].queryIdx != i) {
        throw std::runtime_error(
            "invalid DMatch queryIdx for knn_matches_flann");
      }
      if (knn_matches_flann_cv[i][0].queryIdx != i) {
        throw std::runtime_error(
            "invalid DMatch queryIdx for knn_matches_flann_cv");
      }

      if (knn_matches_flann[i][0].trainIdx ==
          knn_matches_bruteforce[i][0].trainIdx) {
        ++nn_score;
      }

      if (knn_matches_flann[i][1].trainIdx ==
          knn_matches_bruteforce[i][1].trainIdx) {
        ++nn2_score;
      }

      if (knn_matches_flann_cv[i][0].trainIdx ==
          knn_matches_bruteforce[i][0].trainIdx) {
        ++nn_score_cv;
      }

      if (knn_matches_flann_cv[i][1].trainIdx ==
          knn_matches_bruteforce[i][1].trainIdx) {
        ++nn2_score_cv;
      }
    }
  ...
  drawMatches(img1, img2, keypoints1, keypoints2, good_matches_nn,
              "data/debug/test_matching/" + getTestSuiteName() + "_" +
                  getTestName() + "_" + "00_matches_nn.png");

#if ENABLE_MY_MATCHING
  std::cout << "filtering matches by ratio test..." << std::endl;
  std::vector<DMatch> good_matches_ratio;
  phg::DescriptorMatcher::filterMatchesRatioTest(knn_matches_flann,
                                                 good_matches_ratio);
  drawMatches(img1, img2, keypoints1, keypoints2, good_matches_ratio,
              "data/debug/test_matching/" + getTestSuiteName() + "_" +
                  getTestName() + "_" + "01_matches_ratio.png");

  std::cout << "filtering matches by clusters..." << std::endl;
  std::vector<DMatch> good_matches_clusters_only;
  phg::DescriptorMatcher::filterMatchesClusters(
      good_matches_nn, keypoints1, keypoints2, good_matches_clusters_only);
  drawMatches(img1, img2, keypoints1, keypoints2, good_matches_clusters_only,
              "data/debug/test_matching/" + getTestSuiteName() + "_" +
                  getTestName() + "_" + "03_matches_clusters_only.png");

  std::cout << "filtering matches by ratio & clusters" << std::endl;
  std::vector<DMatch> good_matches_clusters_and_ratio;
  phg::DescriptorMatcher::filterMatchesClusters(
      good_matches_ratio, keypoints1, keypoints2,
      good_matches_clusters_and_ratio);
  drawMatches(img1, img2, keypoints1, keypoints2,
              good_matches_clusters_and_ratio,
              "data/debug/test_matching/" + getTestSuiteName() + "_" +
                  getTestName() + "_" + "04_matches_clusters_and_ratio.png");
#else
  std::vector<DMatch> good_matches_clusters_and_ratio;
  phg::filterMatchesGMS(good_matches_nn, keypoints1, keypoints2, img1.size(),
                        img2.size(), good_matches_clusters_and_ratio);
  drawMatches(img1, img2, keypoints1, keypoints2,
              good_matches_clusters_and_ratio,
              "data/debug/test_matching/" + getTestSuiteName() + "_" +
                  getTestName() + "_" + "04_matches_gms.png");
#endif
...
#if ENABLE_MY_MATCHING
  const int ntest = 4;
  std::vector<cv::DMatch> *arrs[ntest] = {&good_matches_nn, &good_matches_ratio,
                                          &good_matches_clusters_only,
                                          &good_matches_clusters_and_ratio};
  double *ptrs[ntest] = {&good_nn, &good_ratio, &good_clusters,
                         &good_ratio_and_clusters};
#else
  const int ntest = 2;
  std::vector<cv::DMatch> *arrs[ntest] = {&good_matches_nn,
                                          &good_matches_clusters_and_ratio};
  ...
  for (int i_test = 0; i_test < ntest; ++i_test) {
    ...
    if (arr.size() < 50) {
      std::cerr << "too few matches: " + std::to_string(arr.size())
                << std::endl;
      continue;
    }
...
void testMatching(const cv::Mat &img1, const cv::Mat &img2,
                  const std::vector<cv::KeyPoint> &keypoints1,
                  const std::vector<cv::KeyPoint> &keypoints2,
                  const cv::Mat &descriptors1, const cv::Mat &descriptors2,
                  double &nn_score, double &nn2_score, double &nn_score_cv,
                  double &nn2_score_cv, double &time_my, double &time_cv,
                  double &time_bruteforce, double &time_bruteforce_gpu,
                  double &good_nn, double &good_ratio, double &good_clusters,
                  double &good_ratio_and_clusters, bool do_bruteforce) {
  evaluateMatching(img1, img2, keypoints1, keypoints2, descriptors1,
                   descriptors2, nn_score, nn2_score, nn_score_cv, nn2_score_cv,
  ...
  std::cout << "good_ratio_and_clusters: " << good_ratio_and_clusters
            << std::endl;
}

void testMatchingMultipleDetectors(const cv::Mat &img1, const cv::Mat &img2,
                                   double &nn_score, double &nn2_score,
                                   double &nn_score_cv, double &nn2_score_cv,
                                   double &time_my, double &time_cv,
                                   double &time_bruteforce,
                                   double &time_bruteforce_gpu, double &good_nn,
                                   double &good_ratio, double &good_clusters,
                                   ...
                                   bool do_bruteforce = true) {
  {
    ...
    testMatching(img1, img2, keypoints1, keypoints2, descriptors1, descriptors2,
                 nn_score, nn2_score, nn_score_cv, nn2_score_cv, time_my,
                 time_cv, time_bruteforce, time_bruteforce_gpu, good_nn,
                 good_ratio, good_clusters, good_ratio_and_clusters,
                 do_bruteforce);
  }
#if ENABLE_MY_DESCRIPTOR
  {
...
TEST(MATCHING, SimpleMatching) {
  ...
  double nn_score, nn2_score, nn_score_cv, nn2_score_cv, time_my, time_cv,
      time_bruteforce, time_bruteforce_gpu, good_nn, good_ratio, good_clusters,
      good_ratio_and_clusters;

  testMatchingMultipleDetectors(img1, img2, nn_score, nn2_score, nn_score_cv,
                                nn2_score_cv, time_my, time_cv, time_bruteforce,
                                time_bruteforce_gpu, good_nn, good_ratio,
                                good_clusters, good_ratio_and_clusters);
...
cv::Mat transformImg(const cv::Mat &img2, double angleDegreesClockwise,
                     double scale) {
  ...
  cv::Mat tmp;
  cv::warpAffine(img2, tmp, M,
                 cv::Size(1.5 * img2.cols * scale, 1.5 * img2.rows * scale));

  cv::Mat transformedImage;
  M = cv::getRotationMatrix2D(cv::Point(tmp.cols / 2, tmp.rows / 2),
                              -angleDegreesClockwise, 1.0);
...
void addNoise(cv::Mat &img2) {
  ...
  cv::add(img2, noise,
          img2); // добавляем к преобразованной картинке гауссиан шума
}
...
void testMatchingTransformWrapper(double angleDegreesClockwise, double scale) {
  ...
  cv::imwrite("data/debug/test_matching/" + getTestSuiteName() + "_" +
                  getTestName() + "_" + "hiking_right_rotated_noise.png",
              img2);
  ...
  testMatchingMultipleDetectors(img1, img2, nn_score, nn2_score, nn_score_cv,
                                nn2_score_cv, time_my, time_cv, time_bruteforce,
                                time_bruteforce_gpu, good_nn, good_ratio,
                                good_clusters, good_ratio_and_clusters, false);
...
int getOrthoScore(const cv::Mat &ortho0, const cv::Mat &ortho1,
                  int threshold_px) {
  using namespace cv;
  ...
  for (const cv::DMatch &match : good_matches) {
    cv::Point2f d =
        keypoints1[match.queryIdx].pt - keypoints2[match.trainIdx].pt;
    if (d.x * d.x + d.y * d.y < threshold_px * threshold_px) {
      ++score;
    }
  }

  drawMatches(ortho0, ortho1, keypoints1, keypoints2, good_matches,
              "data/debug/test_matching/" + getTestSuiteName() + "_" +
                  getTestName() + "_" + "ortho_matches.png");
...
TEST(STITCHING, Orthophoto) {
#if ENABLE_MY_MATCHING
  cv::Mat img1 =
      cv::imread("data/src/test_matching/ortho/IMG_160729_071349_0000_RGB.JPG");
  cv::Mat img2 =
      cv::imread("data/src/test_matching/ortho/IMG_160729_071351_0001_RGB.JPG");
  cv::Mat img3 =
      cv::imread("data/src/test_matching/ortho/IMG_160729_071353_0002_RGB.JPG");
  cv::Mat img4 =
      cv::imread("data/src/test_matching/ortho/IMG_160729_071356_0003_RGB.JPG");
  cv::Mat img5 =
      cv::imread("data/src/test_matching/ortho/IMG_160729_071358_0004_RGB.JPG");

  {
    std::function<cv::Mat(const cv::Mat &, const cv::Mat &)>
        homography_builder = [](const cv::Mat &lhs, const cv::Mat &rhs) {
          return getHomography(lhs, rhs);
        };
    cv::Mat ortho2 = phg::stitchPanorama({img1, img2, img3, img4, img5},
                                         {1, 2, -1, 2, 3}, homography_builder);
    cv::imwrite("data/debug/test_matching/" + getTestSuiteName() + "_" +
                    getTestName() + "_" + "ortho_root2.jpg",
                ortho2);
  }

  int counter = 0;
  std::function<cv::Mat(const cv::Mat &, const cv::Mat &)> homography_builder =
      [&counter](const cv::Mat &lhs, const cv::Mat &rhs) {
        ++counter;
        return getHomography(lhs, rhs);
      };

  cv::Mat ortho = phg::stitchPanorama({img1, img2, img3, img4, img5},
                                      {-1, 0, 1, 2, 3}, homography_builder);
  cv::imwrite("data/debug/test_matching/" + getTestSuiteName() + "_" +
                  getTestName() + "_" + "ortho_root0.jpg",
              ortho);
  ...
  int score = getOrthoScore(
      ortho, cv::imread("data/src/test_matching/ortho/ortho_root0.jpg"),
      threshold_px);
