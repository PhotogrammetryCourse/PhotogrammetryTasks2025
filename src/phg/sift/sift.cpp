...
#define OCTAVE_NLAYERS                                                         \
  3 // в [lowe04] это число промежуточных степеней размытия картинки в рамках
...
#define INITIAL_IMG_SIGMA                                                      \
  0.75 // предполагаемая степень размытия изначальной картинки
#define INPUT_IMG_PRE_BLUR_SIGMA 1.0 // сглаживание изначальной картинки

#define SUBPIXEL_FITTING_ENABLE                                                \
  0 // такие тумблеры включающие/выключающие очередное улучшение алгоритма
...
#define ORIENTATION_NHISTS                                                     \
  36 // число корзин при определении ориентации ключевой точки через гистограммы
#define ORIENTATION_WINDOW_R                                                   \
  3 // минимальный радиус окна в рамках которого будет выбрана ориентиация (в
    // пикселях), R=3 => 5x5 окно
#define ORIENTATION_VOTES_PEAK_RATIO                                           \
  0.80 // 0.8 => если гистограмма какого-то направления получила >= 80% от
       // максимального чиссла голосов - она тоже победила

#define DESCRIPTOR_SIZE 4 // 4x4 гистограммы декскриптора
#define DESCRIPTOR_NBINS                                                       \
  8 // 8 корзин-направлений в каждой гистограмме дескриптора (4х4 гистограммы,
    // каждая по 8 корзин, итого 4x4x8=128 значений в дескрипторе)
#define DESCRIPTOR_SAMPLES_N                                                   \
  4 // 4x4 замера для каждой гистограммы дескриптора (всего гистограмм 4х4)
    // итого 16х16 замеров
#define DESCRIPTOR_SAMPLE_WINDOW_R                                             \
  1.0 // минимальный радиус окна в рамках которого строится гистограмма из 8
...
void phg::SIFT::buildPyramids(const cv::Mat &imgOrg,
  ...
  for (size_t octave = 0; octave < NOCTAVES; ++octave) {
    if (octave == 0) {
      ...
      // бы то ни было дополнительного размытия (сигмы должны совпадать) cv::Mat
      // img = gaussianPyramid[ TODO ].clone(); тут есть очень важный момент, мы
      // должны указать fx=0.5, fy=0.5 иначе при нечетном размере картинка будет
      // не идеально 2 пикселя в один схлопываться - а слегка смещаться
      // cv::resize(img, img, cv::Size( TODO , TODO ), 0.5, 0.5,
      // cv::INTER_NEAREST); gaussianPyramid[octave * OCTAVE_GAUSSIAN_IMAGES +
      // layer] = img;
    }

    //        #pragma omp parallel for // TODO: если выполните TODO про
    //        "размытие из изначального слоя октавы" ниже - раскоментируйте это
    //        распараллеливание, ведь теперь слои считаются независимо (из
    //        самого первого), проверьте что результат на картинках не изменился
    for (ptrdiff_t layer = 1; layer < OCTAVE_GAUSSIAN_IMAGES; ++layer) {
      ...
      double sigma =
          sqrt(sigmaCur * sigmaCur -
               sigmaPrev * sigmaPrev); // sigma2  - сигма которую надо добавить
                                       // чтобы довести sigma1 до sigma12
      ...
      // TODO: переделайте это добавочное размытие с варианта "размываем
      // предыдущий слой" на вариант "размываем самый первый слой октавы до
      // степени размытия сигмы нашего текущего слоя" проверьте - картинки
      // отладочного вывода выглядят один-в-один до/после? (посмотрите на них
      // туда-сюда быстро мигая)

      cv::Mat imgLayer =
          gaussianPyramid[octave * OCTAVE_GAUSSIAN_IMAGES + prevLayer].clone();
  ...
  for (size_t octave = 0; octave < NOCTAVES; ++octave) {
    for (size_t layer = 0; layer < OCTAVE_GAUSSIAN_IMAGES; ++layer) {
      ...
      // TODO: какие ожидания от картинок можно придумать? т.е. как
...
#pragma omp parallel for
  ...
  // TODO: почему OCTAVE_GAUSSIAN_IMAGES=(OCTAVE_NLAYERS + 3) а не например
  // (OCTAVE_NLAYERS + 2)?
...
#pragma omp for
        for (ptrdiff_t j = 1; j < cur.rows - 1; ++j) {
          for (ptrdiff_t i = 1; i + 1 < cur.cols; ++i) {
            ...
            for (int dz = -1; dz <= 1 && (is_min || is_max); ++dz) {
              for (int dy = -1; dy <= 1 && (is_min || is_max); ++dy) {
                for (int dx = -1; dx <= 1 && (is_min || is_max); ++dx) {
                  // TODO проверить является ли наш центр все еще экстремум по
                  // сравнению с соседом DoGs[1+dz].at<float>(j+dy, i+dx) ? (не
                  // забудьте учесть что один из соседов - это мы сами)
                }
              }
            }
            ...
            float dy = 0.0f;
            float dvalue = 0.0f;
            // TODO сделать субпиксельное уточнение (хотя бы через
            // параболу-фиттинг независимо по оси X и оси Y, но лучше через
            // честный ряд Тейлора, матрицу Гессе и итеративное смещение если
            // экстремум оказался в соседнем пикселе)
            ...
            if (contrast <
                contrast_threshold /
                    OCTAVE_NLAYERS) // TODO почему порог контрастности должен
            ...
            float biggestVote;
            int oriRadius =
                (int)(ORIENTATION_WINDOW_R * (1.0 + k * (layer - 1)));
            ...
            for (size_t bin = 0; bin < ORIENTATION_NHISTS; ++bin) {
              ...
              if (value > prevValue && value > nextValue &&
                ...
                // TODO добавьте уточнение угла наклона - может помочь
                // определенная выше функция parabolaFitting(float x0, float x1,
                // float x2)
...
bool phg::SIFT::buildLocalOrientationHists(const cv::Mat &img, size_t i,
  ...
  for (size_t y = j - radius + 1; y < j + radius; ++y) {
    for (size_t x = i - radius + 1; x < i + radius; ++x) {
      ...
      //            rassert(orientation >= 0.0 && orientation < 360.0,
      //            5361615612); static_assert(360 % ORIENTATION_NHISTS == 0,
      //            "Inappropriate bins number!"); size_t bin = TODO rassert(bin
      //            < ORIENTATION_NHISTS, 361236315613); sum[bin] += magnitude;
...
bool phg::SIFT::buildDescriptor(const cv::Mat &img, float px, float py,
  ...
  for (int hstj = 0; hstj < DESCRIPTOR_SIZE;
       ++hstj) { // перебираем строку в решетке гистограмм
    for (int hsti = 0; hsti < DESCRIPTOR_SIZE;
         ++hsti) { // перебираем колонку в решетке гистограмм

      float sum[DESCRIPTOR_NBINS] = {0.0f};

      for (int smpj = 0; smpj < DESCRIPTOR_SAMPLES_N;
           ++smpj) { // перебираем строчку замера для текущей гистограммы
        for (int smpi = 0; smpi < DESCRIPTOR_SAMPLES_N;
             ++smpi) { // перебираем столбик очередного замера для текущей
                       // гистограммы
          for (int smpy = 0; smpy < smpW;
               ++smpy) { // перебираем ряд пикселей текущего замера
            for (int smpx = 0; smpx < smpW;
                 ++smpx) { // перебираем столбик пикселей текущего замера
                           //                            cv::Point2f
              //                            shift(((-DESCRIPTOR_SIZE/2.0 + hsti)
              //                            * DESCRIPTOR_SAMPLES_N + smpi) *
              //                            smpW, TODO);
              //                            std::vector<cv::Point2f>
              //                            shiftInVector(1, shift);
              //                            cv::transform(shiftInVector,
              //                            shiftInVector,
              //                            relativeShiftRotation); //
              //                            преобразуем относительный сдвиг с
              //                            учетом ориентации ключевой точки
              //                            shift = shiftInVector[0];

              //                            int x = (int) (px + shift.x);
              //                            int y = TODO;

              //                            if (y - 1 < 0 || y + 1 > img.rows ||
              //                            x - 1 < 0 || x + 1 > img.cols)
              //                                return false;
              ...
              //                            M_PI; orientation = (orientation
              //                            + 90.0); if (orientation <  0.0)
              //                            orientation += 360.0; if
              //                            (orientation >= 360.0) orientation
              //                            -= 360.0;

              // TODO за счет чего этот вклад будет сравниваться с этим же
              // вкладом даже если эта картинка будет повернута? что нужно
              // сделать с ориентацией каждого градиента из окрестности этой
              // ключевой точки?

              //                            rassert(orientation >= 0.0 &&
              //                            orientation < 360.0, 3515215125412);
              static_assert(360 % DESCRIPTOR_NBINS == 0,
                            "Inappropriate bins number!");
              //                            size_t bin = TODO;
              //                            rassert(bin < DESCRIPTOR_NBINS,
              //                            361236315613); sum[bin] +=
              //                            magnitude;
              // TODO хорошая идея добавить трилинейную интерполяцию как
              // предложено в статье, или хотя бы сэмулировать ее - сгладить
              // получившиеся гистограммы
            }
          }
        }
      }

      // TODO нормализовать наш вектор дескриптор (подсказка: посчитать его
      // длину и поделить каждую компоненту на эту длину)

      float *votes =
          &(descriptor[(hstj * DESCRIPTOR_SIZE + hsti) *
                       DESCRIPTOR_NBINS]); // нашли где будут лежать корзины
                                           // нашей гистограммы
