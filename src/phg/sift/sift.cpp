...
#define HISTOGRAM_SMOOTHING_ENABLE 1
...
void phg::SIFT::buildPyramids(const cv::Mat &imgOrg, std::vector<cv::Mat> &gaussianPyramid, std::vector<cv::Mat> &DoGPyramid) {
    ...
    for (size_t octave = 0; octave < NOCTAVES; ++octave) {
        if (octave == 0) {
            ...
            cv::resize(img, img, automaticKernelSize, 0.5, 0.5, cv::INTER_NEAREST);
            gaussianPyramid[octave * OCTAVE_GAUSSIAN_IMAGES + layer] = img;
        }

#define SIGMA_FROM_INITIAL_LAYER_ENABLED 1

#if SIGMA_FROM_INITIAL_LAYER_ENABLED
#pragma omp parallel for
#endif  // SIGMA_FROM_INITIAL_LAYER_ENABLED
        for (ptrdiff_t layer = 1; layer < OCTAVE_GAUSSIAN_IMAGES; ++layer) {
#if SIGMA_FROM_INITIAL_LAYER_ENABLED
            double sigmaInitial = INITIAL_IMG_SIGMA * pow(2.0, octave);
            double sigmaCur = INITIAL_IMG_SIGMA * pow(2.0, octave) * pow(k, layer);
            double sigmaDiff = sqrt(sigmaCur * sigmaCur - sigmaInitial * sigmaInitial);
            ...
            double sigmaDiff = sqrt(sigmaCur * sigmaCur - sigmaPrev * sigmaPrev);         // sigma2  - сигма которую надо добавить чтобы довести sigma1 до sigma12
            cv::Mat imgLayer = gaussianPyramid[octave * OCTAVE_GAUSSIAN_IMAGES + prevLayer].clone();
#endif  // SIGMA_FROM_INITIAL_LAYER_ENABLED
            cv::GaussianBlur(imgLayer, imgLayer, automaticKernelSize, sigmaDiff, sigmaDiff);
...
float parabolaFitting(float x0, float x1, float x2) {
    ...
    rassert(shift >= -1 && shift <= 1, 507942572018);
    return shift;
}

bool findKeypointSubpixelOffset(
    const std::vector<cv::Mat> &DoGPyramid,
    size_t octave,
    ptrdiff_t orig_y, ptrdiff_t orig_x, ptrdiff_t layer,
    float &dx_final, float &dy_final, float &contrast) {
    ...
    for (int it = 0; it < MAX_ITERS; ++it) {
        if (x < 1 || x + 1 >= cur.cols || y < 1 || y + 1 >= cur.rows)
            return false;

        float dDx = 0.5f * (cur.at<float>(y, x + 1) -
                            cur.at<float>(y, x - 1));
        float dDy = 0.5f * (cur.at<float>(y + 1, x) -
                            cur.at<float>(y - 1, x));
        cv::Vec2f g(dDx, dDy);

        float dDxx = cur.at<float>(y, x + 1) - 2.0f * cur.at<float>(y, x) + cur.at<float>(y, x - 1);
        float dDyy = cur.at<float>(y + 1, x) - 2.0f * cur.at<float>(y, x) + cur.at<float>(y - 1, x);
        float dDxy = 0.25f * (cur.at<float>(y + 1, x + 1) - cur.at<float>(y + 1, x - 1) - cur.at<float>(y - 1, x + 1) + cur.at<float>(y - 1, x - 1));

        cv::Matx22f H(dDxx, dDxy,
                      dDxy, dDyy);

        double det = static_cast<double>(dDxx) * dDyy - static_cast<double>(dDxy) * dDxy;
        if (std::fabs(det) < 1e-6)
            return false;  // hessian is not invertible

        cv::Vec2f offset = H.solve(-g, cv::DECOMP_LU);
        if (std::isnan(offset[0]) || std::isnan(offset[1]))
            return false;

        if (std::abs(offset[0]) <= 0.5f &&
            std::abs(offset[1]) <= 0.5f) {
            dx_final = (x + offset[0]) - orig_x;
            dy_final = (y + offset[1]) - orig_y;

            contrast = cur.at<float>(y, x) + 0.5f * g.dot(offset);
            return true;
        }
...
#pragma omp for
                for (ptrdiff_t j = 1; j < cur.rows - 1; ++j) {
                    for (ptrdiff_t i = 1; i < cur.cols - 1; ++i) {
                        ...
                        for (int dz = -1; dz <= 1 && (is_min || is_max); ++dz) {
                            for (int dy = -1; dy <= 1 && (is_min || is_max); ++dy) {
                                for (int dx = -1; dx <= 1 && (is_min || is_max); ++dx) {
                                    ...
                                    float other = DoGs[1 + dz].at<float>(j + dy, i + dx);
                                    is_min &= other > center;
                                    is_max &= other < center;
                                }
                            }
                        ...
                        {
                            bool refined = findKeypointSubpixelOffset(DoGPyramid, octave, j, i, layer, dx, dy, contrast);
                            if (!refined) {
                                dx = 0.0f;
                                dy = 0.0f;
                                contrast = center;
                        ...
                        for (size_t bin = 0; bin < ORIENTATION_NHISTS; ++bin) {
                            ...
                            if (value > prevValue && value > nextValue && votes[bin] > biggestVote * ORIENTATION_VOTES_PEAK_RATIO) {
                                ...
                                kp.angle = (bin + 0.5f + shift) * (360.0 / ORIENTATION_NHISTS);
...
bool phg::SIFT::buildLocalOrientationHists(const cv::Mat &img, size_t i, size_t j, size_t radius,
    ...
    std::vector<float> sum(ORIENTATION_NHISTS, 0.0f);

    for (size_t y = j - radius + 1; y < j + radius; ++y) {
        for (size_t x = i - radius + 1; x < i + radius; ++x) {
            cv::Vec2f grad{img.at<float>(y, x + 1) - img.at<float>(y, x - 1), img.at<float>(y + 1, x) - img.at<float>(y - 1, x)};
            // m(x, y)=(L(x + 1, y) − L(x − 1, y))^2 + (L(x, y + 1) − L(x, y − 1))^2
            double magnitude = cv::norm(grad);

            // atan( (L(x, y + 1) − L(x, y − 1)) / (L(x + 1, y) − L(x − 1, y)) )
            double orientation = atan2(grad[1], grad[0]);
                ...
                for (int smpi = 0; smpi < DESCRIPTOR_SAMPLES_N; ++smpi) {  // перебираем столбик очередного замера для текущей гистограммы
                    for (int smpy = 0; smpy < smpW; ++smpy) {              // перебираем ряд пикселей текущего замера
                        for (int smpx = 0; smpx < smpW; ++smpx) {          // перебираем столбик пикселей текущего замера
                            ...
                            cv::Point2f shift(((-DESCRIPTOR_SIZE / 2.0 + hsti) * DESCRIPTOR_SAMPLES_N + smpi) * smpW + smpx, ((-DESCRIPTOR_SIZE / 2.0 + hstj) * DESCRIPTOR_SAMPLES_N + smpj) * smpW + smpy);
                            ...
                            rassert(bin < DESCRIPTOR_NBINS, 36123631552613);
                            sum[bin] += magnitude;
#endif
                        }
                    }
