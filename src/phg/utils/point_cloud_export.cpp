...
public:
  DataExporter(cv::Mat outputData, cv::Mat outputColor, std::string outputfile,
...
const char *enum2str[] = {"ascii", "binary_big_endian", "binary_little_endian"};
...
DataExporter::DataExporter(cv::Mat outputData, cv::Mat outputColor,
                           std::string outputfile, FileFormat outputformat)
...
void DataExporter::exportToFile() {
  ...
  const unsigned long numIter =
      3 * numElem; // Number of iteration (3 channels * numElem)
  ...
  case FileFormat::PLY_BIN_BIGEND:
    // Looping through all
    for (unsigned long i = 0; i < numIter;
         i += 3) {                           // Loop through all elements
      for (unsigned int j = 0; j < 3; j++) { // Loop through 3 coordinates
        if (hostIsLittleEndian) {
          bufferXYZ = ReverseFloat(
              pData[i + j]); // Convert from host to network (Big endian)
          filestream.write(reinterpret_cast<const char *>(
                               &bufferXYZ), // Non compiled cast to char array
                           sizeof(bufferXYZ));
        } else {
          bufferXYZ = pData[i + j];
          filestream.write(reinterpret_cast<const char *>(
      ...
      for (int j = 2; j >= 0; j--) {
        // OpenCV uses BGR format, so the order of writing is reverse to comply
        // with the RGB format
        ...
        if (hostIsLittleEndian) {
          filestream.write(
              reinterpret_cast<const char *>(
                  pData + i + j), // Non compiled cast to char array
              sizeof(bufferXYZ));
        } else {
          bufferXYZ = ReverseFloat(pData[i + j]);
          filestream.write(reinterpret_cast<const char *>(&bufferXYZ),
                           sizeof(bufferXYZ));
        }
      }
      for (int j = 2; j >= 0; j--) {
  ...
  case FileFormat::PLY_ASCII:
    for (unsigned long i = 0; i < numIter;
      ...
      for (int j = 2; j >= 0; j--) {
        // OpenCV uses BGR format, so the order of writing is reverse to comply
        // with the RGB format
        filestream << (unsigned short)pColor[i + j]
                   << (j == 0 ? "" : " "); // Loop through RGB
      }
...
DataExporter::~DataExporter() { filestream.close(); }
} // namespace

void phg::exportPointCloud(
    const std::vector<cv::Vec3d> &point_cloud, const std::string &path,
    const std::vector<cv::Vec3b> &point_cloud_colors_bgr) {
  if (!point_cloud_colors_bgr.empty() &&
      point_cloud_colors_bgr.size() != point_cloud.size()) {
    throw std::runtime_error(
        "!point_cloud_colors_bgr.empty() && point_cloud_colors_bgr.size() != "
        "point_cloud.size()");
  }
  ...
  for (int i = 0; i < (int)point_cloud.size(); ++i) {
    ...
    cv::Vec3b c = point_cloud_colors_bgr.empty() ? cv::Vec3b{200, 200, 200}
                                                 : point_cloud_colors_bgr[i];
