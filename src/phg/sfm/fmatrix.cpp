...
void infoF(const cv::Matx33d &Fcv) {
  ...
  Eigen::JacobiSVD<Eigen::MatrixXd> svdf(F, Eigen::ComputeFullU |
                                                Eigen::ComputeFullV);
...
cv::Matx33d estimateFMatrixDLT(const cv::Vec2d *m0, const cv::Vec2d *m1,
  ...
  //        Eigen::ComputeFullV); Eigen::VectorXd null_space = TODO
  //
  ...
  //
  ////             Поправить F так, чтобы соблюдалось свойство фундаментальной
  ///матрицы (последнее сингулярное значение = 0)
  //        Eigen::JacobiSVD<Eigen::MatrixXd> svdf(F, Eigen::ComputeFullU |
  //        Eigen::ComputeFullV);
  //
  //          TODO
...
cv::Matx33d estimateFMatrixRANSAC(const std::vector<cv::Vec2d> &m0,
  ...
  //            int support = 0;
  //            for (int i = 0; i < n_matches; ++i) {
  //                if (phg::epipolarTest(m0[i], m1[i], todo, threshold_px) &&
  //                phg::epipolarTest(m1[i], m0[i], todo, threshold_px))
  //                {
  //                    ++support;
  //                }
  //            }
  ...
  //                best_F = F;
  //
  //                std::cout << "estimateFMatrixRANSAC : support: " <<
  //                best_support << "/" << n_matches << std::endl; infoF(F);
  //
  ...
  //        }
  //
  //        std::cout << "estimateFMatrixRANSAC : best support: " <<
  //        best_support << "/" << n_matches << std::endl;
  //
  //        if (best_support == 0) {
  //            throw std::runtime_error("estimateFMatrixRANSAC : failed to
