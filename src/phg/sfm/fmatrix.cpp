...
// get transform matrix so that after transform centroid of points will be zero
// and Root Mean Square distance from centroid will be sqrt(2)
cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d> &m,
  ...
  if (verbose) {
    std::cout << "NORMALIZE TRANSFORM: centroid = " << centroid
              << ", scale = " << s << std::endl;
  }

  cv::Matx33d T(s, 0.0, -s * centroid[0], 0.0, s, -s * centroid[1], 0.0, 0.0,
                1.0);
...
cv::Matx33d estimateFMatrixRANSAC(const std::vector<cv::Vec2d> &m0,
                                  ...
                                  double threshold_px, bool verbose = true) {
  if (m0.size() != m1.size()) {
  ...
  {
    //             check log: centroid should become close to zero, scale close
    //             to 1
...
cv::Matx33d phg::findFMatrix(const std::vector<cv::Vec2d> &m0,
                             const std::vector<cv::Vec2d> &m1,
                             double threshold_px, bool verbose) {
