...
namespace {
    ...
    //             Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
        ...
        auto singular = svdf.singularValues();
        F = svdf.matrixU() * Eigen::Vector3d{singular[0], singular[1], 0}.asDiagonal() * svdf.matrixV().transpose();
    ...
    cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d> &m)
    {
        ...
        double root_mean_square = std::sqrt(mean_square) / (double) m.size();
        double scale = std::sqrt(2) / root_mean_square;
...
//             Проверьте лог: при повторной нормализации должно найтись почти единичное преобразование
            rassert(cv::norm(getNormalizeTransform(m0_t) - cv::Matx33d::eye()) < 1e-3, 34312912321);
            rassert(cv::norm(getNormalizeTransform(m1_t) - cv::Matx33d::eye()) < 1e-3, 34312912322);
        }
//        // https://en.wikipedia.org/wiki/Random_sample_consensus#Parameters
//        // будет отличаться от случая с гомографией
        constexpr int n_trials = 14000;
