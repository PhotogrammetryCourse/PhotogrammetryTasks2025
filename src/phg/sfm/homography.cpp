...
// очень важно при выполнении метода гаусса использовать выбор опорного
// элемента: об этом можно почитать в источнике кода или на вики:
...
// см. Hartley, Zisserman: Multiple View Geometry in Computer Vision. Second
// Edition 4.1, 4.1.2
...
cv::Mat estimateHomographyRANSAC(const std::vector<cv::Point2f> &points_lhs,
  ...
  if (points_lhs.size() != points_rhs.size()) {
    throw std::runtime_error(
        "findHomography: points_lhs.size() != points_rhs.size()");
  }

  // TODO Дополнительный балл, если вместо обычной версии будет использована
  // модификация a-contrario RANSAC
  // * [1] Automatic Homographic Registration of a Pair of Images, with A
  // Contrario Elimination of Outliers. (Lionel Moisan, Pierre Moulon, Pascal
  // Monasse)
  // * [2] Adaptive Structure from Motion with a contrario model estimation.
  // (Pierre Moulon, Pascal Monasse, Renaud Marlet)
  // * (простое описание для понимания)
  // * [3]
  // http://ikrisoft.blogspot.com/2015/01/ransac-with-contrario-approach.html
  ...
  //
  //            cv::Mat H = estimateHomography4Points(points_lhs[sample[0]],
  //            points_lhs[sample[1]], points_lhs[sample[2]],
  //            points_lhs[sample[3]],
  //                                                  points_rhs[sample[0]],
  //                                                  points_rhs[sample[1]],
  //                                                  points_rhs[sample[2]],
  ...
  //                try {
  //                    cv::Point2d proj =
  //                    phg::transformPoint(points_lhs[i_point], H); if
  //                    (cv::norm(proj - cv::Point2d(points_rhs[i_point])) <
  ...
  //                best_H = H;
  //
  //                std::cout << "estimateHomographyRANSAC : support: " <<
  //                best_support << "/" << n_matches << std::endl;
  //
  ...
  //        }
  //
  //        std::cout << "estimateHomographyRANSAC : best support: " <<
  //        best_support << "/" << n_matches << std::endl;
  //
  //        if (best_support == 0) {
  //            throw std::runtime_error("estimateHomographyRANSAC : failed to
  //            estimate homography");
  //        }
  //
  //        return best_H;
...
cv::Mat phg::findHomography(const std::vector<cv::Point2f> &points_lhs,
                            const std::vector<cv::Point2f> &points_rhs) {
  return estimateHomographyRANSAC(points_lhs, points_rhs);
}

// чтобы заработало, нужно пересобрать библиотеку с дополнительным модулем
// calib3d (см. инструкцию в корневом CMakeLists.txt)
cv::Mat phg::findHomographyCV(const std::vector<cv::Point2f> &points_lhs,
                              const std::vector<cv::Point2f> &points_rhs) {
  return cv::findHomography(points_lhs, points_rhs, cv::RANSAC);
}

// T - 3x3 однородная матрица, например, гомография
// таким преобразованием внутри занимается функции cv::perspectiveTransform и
// cv::warpPerspective
