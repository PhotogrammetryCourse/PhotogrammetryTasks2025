...
cv::Mat estimateHomography4Points(const cv::Point2f &l0, const cv::Point2f &l1,
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    double w1 = ws1[i];
    A.push_back(
        {0, 0, 0, -x0 * w1, -y0 * w1, -w0 * w1, x0 * y1, y0 * y1, -w0 * y1});
    A.push_back(
        {x0 * w1, y0 * w1, w0 * w1, 0, 0, 0, -x0 * x1, -y0 * x1, w0 * x1});
  }
...
cv::Mat estimateHomographyRANSAC(const std::vector<cv::Point2f> &points_lhs,
  ...
  for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
    ...
    cv::Mat H = estimateHomography4Points(
        points_lhs[sample[0]], points_lhs[sample[1]], points_lhs[sample[2]],
        points_lhs[sample[3]], points_rhs[sample[0]], points_rhs[sample[1]],
    ...
    for (int i_point = 0; i_point < n_matches; ++i_point) {
      try {
        ...
        if (cv::norm(proj - cv::Point2d(points_rhs[i_point])) <
  ...
  if (best_support == 0) {
    throw std::runtime_error(
        "estimateHomographyRANSAC : failed to estimate homography");
  }
