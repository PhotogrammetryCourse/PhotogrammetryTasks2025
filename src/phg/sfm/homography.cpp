...
cv::Mat estimateHomographyRANSAC(const std::vector<cv::Point2f> &points_lhs, const std::vector<cv::Point2f> &points_rhs) {
    ...
    const float confidence = 0.999f;
    const float inliers_ratio = 0.5f;
    const int n_trials = log(1 - confidence) / log(1 - pow(inliers_ratio, n_samples));
...
cv::Point2d phg::transformPoint(const cv::Point2d &pt, const cv::Mat &T) {
    if (T.rows != 3 || T.cols != 3)
        throw std::runtime_error("phg::transformPoint : T must be 3x3");
    cv::Vec3d pt3d{pt.x, pt.y, 1};
    const cv::Mat proj = T * pt3d;
    double x = proj.at<double>(0);
    double y = proj.at<double>(1);
