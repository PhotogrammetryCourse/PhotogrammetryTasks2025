...
matrix34d canonicalizeP(const matrix34d &P) {
  ...
  Eigen::MatrixXd RRe;
  copy(RR, RRe);
  Eigen::JacobiSVD<Eigen::MatrixXd> svd(RRe, Eigen::ComputeFullU |
                                                 Eigen::ComputeFullV);
...
cv::Matx34d estimateCameraMatrixRANSAC(const phg::Calibration &calib,
  ...
  throw std::runtime_error("not implemented yet");
  //        if (X.size() != x.size()) {
  //            throw std::runtime_error("estimateCameraMatrixRANSAC: X.size()
  //            != x.size()");
  //        }
  //
  ...
  //                cv::Vec2d px = TODO спроецировать 3Д точку в пиксель с
  //                использованием P и calib; if (cv::norm(px - x[i]) <
  //                threshold_px) {
  //                    ++support;
  //                }
  //            }
  ...
  //                best_P = P;
  //
  //                std::cout << "estimateCameraMatrixRANSAC : support: " <<
  //                best_support << "/" << n_points << std::endl;
  //
  ...
  //        }
  //
  //        std::cout << "estimateCameraMatrixRANSAC : best support: " <<
  //        best_support << "/" << n_points << std::endl;
  //
  //        if (best_support == 0) {
  //            throw std::runtime_error("estimateCameraMatrixRANSAC : failed to
