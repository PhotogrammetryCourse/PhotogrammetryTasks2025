...
namespace {
    ...
    cv::Matx34d estimateCameraMatrixDLT(const cv::Vec3d *Xs, const cv::Vec3d *xs, int count)
    {
        ...
        for (int i = 0; i < count; ++i) {
            ...
            A.block(i * 2, 0, 2, 12) 
                << 0,     0,     0,     0,     -X * w, -Y * w, -Z * w, -W * w,  y * X,  y * Y,  y * Z,  y * W,
                   X * w, Y * w, Z * w, W * w,  0,      0,      0,      0,     -x * X, -x * Y, -x * Z, -x * W;
        }

        
        auto eigen_result = Eigen::JacobiSVD(A, Eigen::ComputeFullV).matrixV().col(A.cols() - 1);
        matrix34d result(eigen_result.data());
    ...
    cv::Matx34d estimateCameraMatrixRANSAC(const phg::Calibration &calib, const std::vector<cv::Vec3d> &X, const std::vector<cv::Vec2d> &x)
    {
        ...
        for (int i_trial = 0; i_trial < n_trials; ++i_trial) {
            ...
            for (int i = 0; i < n_points; ++i) {
                cv::Vec3d projection = calib.project(P * cv::Vec4d{X[i][0], X[i][1], X[i][2], 1});
                cv::Vec2d px = {projection[0] / projection[2], projection[1] / projection[2]};
