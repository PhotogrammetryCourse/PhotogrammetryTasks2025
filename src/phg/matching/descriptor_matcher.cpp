...
void phg::DescriptorMatcher::filterMatchesRatioTest(const std::vector<std::vector<cv::DMatch>> &matches,
    ...
    constexpr float THRESHOLD_RATIO = 0.7f;
    for (auto &match : matches) {
        if (match.size() >= 2 && match[0].distance < match[1].distance * THRESHOLD_RATIO)
...
void phg::DescriptorMatcher::filterMatchesClusters(const std::vector<cv::DMatch> &matches,
    ...
    for (int i = 0; i < n_matches; ++i) {
        std::vector<int> neigh_query, neigh_train;
        for (int k = 0; k < total_neighbours; ++k) {
            if (distances2_query.at<float>(i, k) < radius2_query)
                neigh_query.push_back(indices_query.at<int>(i, k));

            if (distances2_train.at<float>(i, k) < radius2_train)
                neigh_train.push_back(indices_train.at<int>(i, k));
        }

        auto common = count_if(neigh_query.begin(), neigh_query.end(), [&](const int neighbour) {
            return std::find(neigh_train.begin(), neigh_train.end(), neighbour) != neigh_train.end();
        });
